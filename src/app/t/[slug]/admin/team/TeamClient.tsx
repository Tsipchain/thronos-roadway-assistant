...
export default function TeamClient({
  ...
  const saveEdit = async (tech: Tech) => {
    ...
    if (res.ok) {
      setTechs((prev) =>
        prev.map((t) => (t.id === tech.id ? { ...t, user: { ...t.user, ...editData } } : t))
      );
      setEditing(null);
  ...
  const resetPassword = async (tech: Tech) => {
    ...
    setSaving(true);
    const res = await fetch(`/api/t/${tenantSlug}/team/${tech.id}/reset-password`, {
      method: "POST",
    });
    setSaving(false);
    if (res.ok) {
      const data = await res.json();
      setNewPassword({ techId: tech.id, password: data.newPassword });
    } else {
      setMsg("❌ Σφάλμα επαναφοράς");
    }
  };
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3 text-sm">
            ...
            <h1 className="text-xl font-bold">👥 Ομάδα Τεχνικών</h1>
          </div>
          <button
        ...
        {showAdd && (
          <div className="bg-white/5 border border-purple-500/30 rounded-2xl p-5 mb-6">
            ...
            <button
              ...
              disabled={addingTech || !addData.name || !addData.phone || !addData.password}
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  ...
                  {editing !== tech.id && (
                    <button
                      ...
                      className="text-xs text-slate-400 hover:text-white transition bg-white/5 px-3 py-1.5 rounded-lg"
                    >
                      ✏️ Επεξ.
                    </button>
                  )}
              ...
              {editing === tech.id && (
                <div className="mt-4 border-t border-white/10 pt-4 space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => { setEditing(null); setNewPassword(null); }}
